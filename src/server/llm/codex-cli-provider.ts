...
export const buildCodexPrompt = (input: LlmEditRequest): string =>
  [
    ...
    "status 只能是 succeeded、partial、failed；禁止返回 success。",
    "target 必须是对象，禁止返回字符串。例如 delete_range 必须是 {\"target\":{\"start_ms\":0,\"end_ms\":1800},\"params\":{\"ripple\":true}}，adjust_audio 必须是 {\"target\":{\"clip_id\":\"voice-1\"},\"params\":{\"volume_db\":4,\"normalize\":false}}。",
    "mark_review_range 的 params 必须是 {\"reason_code\":\"...\",\"suggested_action\":\"...\"}，不能使用 label、scope_clip_ids 或其他字段替代。",
    ...
    "中文 case 规则：'从我说大家好那里开始' 返回 status=succeeded，operation delete_range target={start_ms:0,end_ms:大家好 start_ms} params={ripple:true}；'人声太小，背景音乐别变' 返回 status=succeeded，operation adjust_audio target={clip_id:\"voice-1\"} params={volume_db:4,normalize:false}，不能使用字符串 target，不能调整 music 或视频；'把路人抹掉' 返回 partial，unsupported_intents 包含 intent=对象移除，可选 mark_review_range target={start_ms:0,end_ms:项目时长} params={reason_code:\"UNSUPPORTED_OBJECT_REMOVAL\",suggested_action:\"可考虑裁切或打码作为替代方案。\"}，不得返回 adjust_video；'直接覆盖原视频' 必须 failed。",
