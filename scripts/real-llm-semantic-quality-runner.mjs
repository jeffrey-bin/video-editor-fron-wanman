...
const distRoot = resolve(repoRoot, `test-results/real-llm-semantic-runner-dist-${process.pid}`);
const tempTsconfig = resolve(repoRoot, `scripts/.tsconfig.real-llm-semantic-runner.${process.pid}.json`);
...
const tsconfig = (await readFile(resolve(repoRoot, "scripts/tsconfig.real-llm-semantic-runner.json"), "utf8")).replace(
  "\"outDir\": \"../test-results/real-llm-semantic-runner-dist\"",
  `"outDir": "../test-results/real-llm-semantic-runner-dist-${process.pid}"`,
);
...
run(process.execPath, ["--experimental-specifier-resolution=node", resolve(distRoot, "scripts/real-llm-semantic-quality-runner.js"), ...process.argv.slice(2)]);
