...
const baseRequest = (
  prompt: string,
  clips: LlmEditRequest["context"]["clips"] = [{ id: "clip", trackId: "video_main", kind: "video", startMs: 0, endMs: 10000, assetId: "asset" }],
): LlmEditRequest => ({
...
describe("mock provider", () => {
  it("turns prompt intents into deterministic P0 operations", async () => {
    const plan = await generateMockEditPlan(
      baseRequest("剪掉开头 3 秒空白，增强人声，让画面更明亮，添加字幕", [
        { id: "clip", trackId: "video_main", kind: "video", startMs: 0, endMs: 10000, assetId: "asset" },
        { id: "audio", trackId: "audio_main", kind: "audio", startMs: 0, endMs: 10000, assetId: "asset_audio" },
      ]),
    );
